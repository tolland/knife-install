...
module \Install
  class \Install < Chef::Knife
    ...
    option :all_cookbooks,
      :short => "-a",
      :long => "--all-cookbooks",
      :description => "Show all cookbook references, including those from install_recipies if available on nodes from helper cookbook"
    ...
    option :install_cookbook,
      ...
      :description => "Install knife_Install helper cookbook into current chef cookbook directory"
    ...
    def run
      ...
      if config[:install_cookbook]
        ...
        source_path = File.dirname(__FILE__) + "/knife_install_cookbook"
        dest_path = config[:cookbook_path].first + "/knife_install"

        if File.exist?(dest_path)
          ui.msg("knife_install cookbook already present in cookbook directory #{config[:cookbook_path].first} - aborting...")
        else
          FileUtils.copy_entry(source_path, dest_path)
          ui.msg("knife-install cookbook copied to Chef cookbook directory #{config[:cookbook_path].first}")
        end
      ...
      node_list.each do |node|
        ...
        if (config[:all_cookbooks] || config[:totals])
          recipes = (node["knife_Install"] && node["knife_Install"]["seen_recipes"].keys) || node.expand!.recipes.to_a
          if node["knife_Install"] && node["knife_Install"]["seen_recipes"] 
      ...
      unless config[:totals]
        ui.msg("Cookbook Install from node runlists:")
      ...
      if config[:all_cookbooks]
        ...
        ui.msg("Cookbook Install from seen_recipes:")
      ...
      if config[:totals]
        ...
        ui.msg("Cookbook Install totals - runlist-only nodes + seen_recipes:")
